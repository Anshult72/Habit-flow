...
export default function Habits() {
  ...
  const [formData, setFormData] = useState({ name: '', category: 'Health', color: '#FF6B2C', goal: 30, difficulty: 'Medium' });
  ...
  const templates = [
    { name: 'Monk Mode', category: 'Deep Work', color: '#E85D04', goal: 30, difficulty: 'Elite' },
    { name: 'Dopamine Detox', category: 'Detox', color: '#c2410c', goal: 30, difficulty: 'Hard' },
    { name: 'Fitness Routine', category: 'Fitness', color: '#FF6B2C', goal: 20, difficulty: 'Medium' },
    { name: 'Study System', category: 'Learning', color: '#f97316', goal: 25, difficulty: 'Hard' },
    { name: 'Morning Routine', category: 'Productivity', color: '#fb923c', goal: 28, difficulty: 'Medium' },
  ];
  ...
  const activateBundle = (bundle) => {
    const bundleHabits = [
      { name: 'Cold Shower', category: 'Health', color: '#FF6B2C', goal: 30, difficulty: 'Hard' },
      { name: 'Morning Journal', category: 'Mindfulness', color: '#E85D04', goal: 30, difficulty: 'Easy' },
      { name: 'Deep Meditation', category: 'Mindfulness', color: '#FF8C42', goal: 30, difficulty: 'Medium' }
    ];
    bundleHabits.forEach(h => addHabit({ id: Date.now().toString() + Math.random(), ...h }));
  ...
  const handleOpenModal = (habit = null) => {
    if (habit) {
      ...
      setFormData({ name: habit.name, category: habit.category, color: habit.color, goal: habit.goal, difficulty: habit.difficulty || 'Medium' });
    } else {
      setEditingId(null);
      setFormData({ name: '', category: 'Health', color: '#FF6B2C', goal: 30, difficulty: 'Medium' });
    }
    setIsModalOpen(true);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AnimatePresence>
          {filteredHabits.map((habit, i) => (
            ...
            >
              <div className="absolute top-0 right-0 w-32 h-32 opacity-10 rounded-full blur-3xl transform translate-x-10 -translate-y-10 group-hover:opacity-30 transition-opacity" style={{ backgroundColor: habit.color }} />
              <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="flex items-start gap-4">
                  <div className="w-4 h-4 rounded-full mt-1" style={{ backgroundColor: habit.color, boxShadow: `0 0 15px ${habit.color}` }} />
                  <div>
                    <h3 className="font-display font-bold text-xl text-white mb-1">{habit.name}</h3>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] px-2 py-0.5 bg-white/5 border border-white/10 rounded text-textMuted uppercase tracking-wide">{habit.category}</span>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSave} className="space-y-6">
                ...
                <div>
                  <label className="block text-xs font-bold mb-3 text-textMuted uppercase tracking-[0.2em]">Aura Color</label>
