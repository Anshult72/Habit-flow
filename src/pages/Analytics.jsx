...
import {
  ...
  subDays, startOfWeek, addDays, getYear, getMonth, eachMonthOfInterval,
...
import { Activity, Flame, Target, TrendingUp, Monitor, Award, ChevronDown } from 'lucide-react';
  ...
  const radarData = {
    ...
    datasets: [{
      ...
      data: Object.keys(categories).map(cat => {
        ...
        let done = 0;
        catHabits.forEach(h => days.forEach(d => { if (completions[`${h.id}-${d}`]) done++; }));
  ...
  return (
    <div className="space-y-8 pb-10">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <p className="text-[10px] text-textMuted font-bold uppercase tracking-widest">{kpi.label}</p>
                <Icon size={16} style={{ color: kpi.color }} />
              </div>
              <p className="text-3xl font-display font-bold text-white">
                {kpi.value}<span className="text-base text-textMuted">{kpi.suffix}</span>
              </p>
            </motion.div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 relative z-10">
        ...
        >
          <p className="text-xs text-textMuted font-bold uppercase tracking-widest">Productivity Score</p>
          <div className="relative w-36 h-36">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p className="text-[10px] text-textMuted uppercase tracking-wider">/ 100</p>
            </div>
          </div>
      ...
      {/* Row: Distribution + Consistency Web */}
      ...
      <div className="glass-card p-8 rounded-3xl border border-white/5 relative z-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <span className="text-[10px] text-textMuted uppercase tracking-widest border border-white/10 px-3 py-1 rounded-full">This Month</span>
        </div>
        <div className="space-y-4">
          ...
          {habitRankings.length === 0 && (
            <p className="text-textMuted text-center py-8">No habits to rank yet. Create some habits to see analytics.</p>
          )}
        </div>
      </div>
      ...
      <div className="glass-card p-8 rounded-3xl border border-white/5 relative z-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-textMuted text-sm mt-1">Track your digital usage patterns</p>
          </div>
          <div className="glass-card px-6 py-3 rounded-2xl border border-white/5">
            <p className="text-[10px] text-textMuted uppercase tracking-widest mb-1">7-Day Avg</p>
          ...
          {last7.map((d, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-textMuted">{format(d, 'EEE')}</span>
                <span className="text-xs font-bold text-white">{hours > 0 ? `${hours}h` : '–'}</span>
              </div>
            );
          })}
        </div>
        <div className="flex gap-6 mt-6 text-xs text-textMuted">
