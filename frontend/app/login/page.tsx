...
import { Mail, Lock, Eye, EyeOff, Zap, ArrowRight, Loader2, Key } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || (!password && !resetMode)) return;
    ...
    try {
      ...
      if (error) {
        // Handle "Account not found" explicitly if it's returned, or simulate it.
        // Supabase typically returns "Invalid login credentials" for both to prevent enumeration.
        ...
        const hasOnboarded = typeof window !== 'undefined' && localStorage.getItem('hasCompletedOnboarding') === 'true';
      ...
      >
        <div className="mb-6 text-center">
          <h2 className="text-xl font-bold text-white mb-1">
            {resetMode ? 'Reset Password' : 'Sign in to HabitFlow'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Reset Password Button (Appears after 1 failed attempt) */}
          {failedAttempts >= 1 && !resetMode && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          <button
            type="submit"
            disabled={loading || !email || (!password && !resetMode)}
