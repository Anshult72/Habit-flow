...
export default function Analytics() {
  ...
  const doughnutData = {
    ...
    datasets: [{
      ...
      backgroundColor: Object.keys(categories).map(cat => CATEGORY_COLORS[cat] || '#FF6B2C'),
      borderWidth: 0,
      hoverOffset: 12,
    }],
  };
  ...
  const tabs = ['monthly', 'weekly', 'yearly', 'screen', 'trajectory'];
  const tabLabels = { monthly: 'Monthly', weekly: 'Weekly', yearly: 'Yearly', screen: 'Screen Time', trajectory: 'Trajectory' };
          ...
          <div className="p-8 h-72">
            <AnimatePresence mode="wait">
              ...
              {activeTab === 'trajectory' && (
                <motion.div key="trajectory" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full">
                  <Line
                    data={{
                      ...
                      datasets: [{
                        ...
                        data: Array.from({ length: 30 }, (_, i) => {
                          ...
                          return habits.filter(h => completions[`${h.id}-${ds}`]).length;
                        }),
                        fill: true,
                        backgroundColor: ctx => { const g = ctx.chart.ctx.createLinearGradient(0,0,0,300); g.addColorStop(0,'rgba(255,107,44,0.35)'); g.addColorStop(1,'rgba(255,107,44,0)'); return g; },
                        borderColor: '#FF6B2C',
                        borderWidth: 2.5,
                        tension: 0.4,
                        pointBackgroundColor: '#FF6B2C',
                    ...
                    options={{ ...chartBase, scales: { y: { grid: { color: 'rgba(255,255,255,0.03)' }, ticks: { color: '#94A3B8' } }, x: { grid: { display: false }, ticks: { color: '#94A3B8', maxTicksLimit: 8 } } } }}
                  />
                </motion.div>
              )}
              {activeTab === 'yearly' && (
