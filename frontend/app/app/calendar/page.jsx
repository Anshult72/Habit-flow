...
import {
  ...
  subMonths, addMonths, subDays
} from 'date-fns';
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, X, FileText, Monitor, LayoutGrid, Table } from 'lucide-react';
...
export default function CalendarPage() {
  ...
  const handleToggle = (habitId, dateStr) => {
    const isCompleting = !completions[`${habitId}-${dateStr}`];
    toggleCompletion(habitId, dateStr);
    if (isCompleting) {
      addXP(15);
      toast.success('Objective Synchronized! +15 XP');
    } else {
      addXP(-15);
    }
  };
    ...
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
        <div>
          ...
          <p className="text-text-muted mt-2 text-lg">Comprehensive habit tracking across time.</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex bg-white/5 border border-white/10 rounded-xl p-1">
            <button
              onClick={() => setViewMode('grid')}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-all ${viewMode === 'grid' ? 'bg-[#FF6B2C] text-white shadow-[0_0_15px_rgba(255,107,44,0.4)]' : 'text-text-muted hover:text-white'}`}
            >
              <LayoutGrid size={14} /> Grid
            </button>
            <button
              onClick={() => setViewMode('table')}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-all ${viewMode === 'table' ? 'bg-[#FF6B2C] text-white shadow-[0_0_15px_rgba(255,107,44,0.4)]' : 'text-text-muted hover:text-white'}`}
            >
              <Table size={14} /> Table
            </button>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4 relative z-10">
        {[
          ...
          >
            <p className="text-[10px] text-text-muted uppercase tracking-widest mb-2">{stat.label}</p>
          ...
          <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative z-10">
            <div className="glass-card rounded-3xl border border-white/5 overflow-hidden">
              <div className="grid grid-cols-7 border-b border-white/5">
                {WEEKDAYS.map(day => (
                  <div key={day} className="py-3 text-center text-[11px] font-bold text-text-muted uppercase tracking-widest">
                    {day}
                  </div>
                ))}
              </div>
                      ...
                      {cell.completedHabits.length > 0 && (
                        <div className="flex flex-wrap gap-0.5 mt-1">
                          ...
                          {cell.completedHabits.length > 4 && (
                            <span className="text-[9px] text-text-muted">+{cell.completedHabits.length - 4}</span>
                          )}
                        </div>
                      )}
            ...
            <div className="flex gap-6 mt-4 text-xs text-text-muted flex-wrap">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                  <thead className="bg-white/[0.02] border-b border-white/10">
                    <tr>
                      <th className="p-4 text-left font-bold text-text-muted uppercase tracking-wider text-xs w-48 sticky left-0 bg-background/90 backdrop-blur-md z-20 border-r border-white/10">Protocol</th>
                      {allDaysStr.map((d, i) => {
                        ...
                        return (
                          <th
                            ...
                            className={`p-1.5 text-center min-w-[36px] border-l border-white/5 cursor-pointer hover:bg-white/5 transition-colors text-text-muted
                      ...
                      <th className="p-4 text-center font-bold text-text-muted uppercase tracking-wider text-xs min-w-[80px] sticky right-0 bg-background/90 backdrop-blur-md z-20 border-l border-white/10">Yield</th>
                    </tr>
                  </thead>
                  <tbody>
                    {habits.map(habit => {
                      ...
                      return (
                        <tr key={habit.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                          ...
                          <td className="p-3 text-center sticky right-0 bg-background/90 backdrop-blur-md z-20 border-l border-white/10 group-hover:bg-background transition-colors">
                            ...
                            <span className="text-[10px] text-text-muted block">/{habit.goal}</span>
                          </td>
                        </tr>
                      );
                    })}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex justify-between items-start mb-8">
                ...
                <button onClick={() => setSelectedDate(null)} className="text-text-muted hover:text-white p-2 rounded-full hover:bg-white/5 transition-colors">
                  <X size={22} />
                </button>
              </div>

              <div className="mb-6">
                <p className="text-xs text-text-muted font-bold uppercase tracking-widest mb-3">Habits</p>
              ...
              <form onSubmit={saveDayData} className="space-y-5">
                <div>
                  <label className="flex items-center gap-2 text-xs font-bold mb-2 text-text-muted uppercase tracking-widest">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#FF6B2C] transition-all text-sm placeholder:text-text-muted/40 resize-none"
