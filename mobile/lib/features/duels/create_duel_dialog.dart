...
class _CreateDuelDialogState extends ConsumerState<CreateDuelDialog> {
  ...
  final _entryXpController = TextEditingController(text: '50');
  ...
  Future<void> _submit() async {
    ...
    final entryXp = int.tryParse(_entryXpController.text) ?? 50;
    final duration = int.tryParse(_durationController.text) ?? 7;

    if (targetId.isEmpty || targetId.length != 8) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a valid 8-digit Opponent ID')),
      );
      return;
    }
    ...
    try {
      await ref.read(duelServiceProvider).createChallenge(targetId, entryXp, duration);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('INITIATE DUEL', style: GoogleFonts.outfit(fontSize: 16.sp, fontWeight: FontWeight.w800, color: AppTheme.textMain, letterSpacing: 1.5)),
          SizedBox(height: 24.h),

          _buildInput('OPPONENT ID (8 DIGITS)', _targetIdController, TextInputType.number),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _isLoading
                  ...
                  : Text('SEND CHALLENGE', style: GoogleFonts.outfit(fontSize: 14.sp, fontWeight: FontWeight.w700, letterSpacing: 1)),
            ),
          ),
        ],
      ),
